...
function eveningshade_enqueue() {
    global $wp_styles;
    wp_enqueue_style( 'themestyle', get_stylesheet_uri() );
    wp_enqueue_style( 'ie7', get_template_directory_uri().'/css/ie7.css',array('themestyle') );
    $wp_styles->add_data( 'ie7', 'conditional', 'IE 7' );

    wp_enqueue_script( 'customjs', get_stylesheet_directory_uri() . '/js/frontend.js', array('jquery'), '1.0.0', true );
}
add_action( 'wp_enqueue_scripts', 'eveningshade_enqueue' );
...
add_action( 'tgmpa_register', 'eveningshade_register_required_plugins' );
function eveningshade_register_required_plugins() {
    $plugins = array(
        array(
            'name'               => 'Evening Shade Shortcodes',
            'slug'               => 'eveningshade-shortcode',
            'source'             => 'eveningshade-shortcode.zip',
            'required'           => true,
            'version'            => '1.0.0',
        )
    );
    ...
    $config = array(
        ...
        'menu'         => 'eveningshade-install-plugins',
        'has_notices'  => true,
        'dismissable'  => true,
        'dismiss_msg'  => '',
        'is_automatic' => false,
        ...
        'strings'      => array(
            'page_title'                      => __( 'Install Required Plugins', 'eveningshade' ),
            'menu_title'                      => __( 'Install Plugins', 'eveningshade' ),
            'installing'                      => __( 'Installing Plugin: %s', 'eveningshade' ),
            'oops'                            => __( 'Something went wrong with the plugin API.', 'eveningshade' ),
            ...
            'return'                          => __( 'Return to Required Plugins Installer', 'eveningshade' ),
            'plugin_activated'                => __( 'Plugin activated successfully.', 'eveningshade' ),
            'complete'                        => __( 'All plugins installed and activated successfully. %s', 'eveningshade' ),
            'nag_type'                        => 'updated'
        )
    );
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'menu_title'                     => __( 'Install Plugins', 'eveningshade' ),
                'installing'                     => __( 'Installing Plugin: %s', 'eveningshade' ),
                'oops'                           => __( 'Something went wrong.', 'eveningshade' ),
                ...
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'eveningshade' ),
                'dashboard'                      => __( 'Return to the dashboard', 'eveningshade' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'eveningshade' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'eveningshade' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'eveningshade' ),
                'dismiss'                        => __( 'Dismiss this notice', 'eveningshade' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'eveningshade' ) . '">' . __( 'Return to the Dashboard', 'eveningshade' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'tgmpa', 'eveningshade', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'tgmpa', 'eveningshade', $rendered, $nag_class );
                }
            }
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'eveningshade' );
                        // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'eveningshade' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'eveningshade' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'eveningshade' ) : __( 'Recommended', 'eveningshade' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'eveningshade' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'eveningshade' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'eveningshade' ) . ' %2$s">' . __( 'Install', 'eveningshade' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'eveningshade' ) . ' %2$s">' . __( 'Activate', 'eveningshade' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'eveningshade' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'eveningshade' ),
                'source' => __( 'Source', 'eveningshade' ),
                'type'   => __( 'Type', 'eveningshade' ),
                'status' => __( 'Status', 'eveningshade' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'eveningshade' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'eveningshade' ),
            );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'eveningshade' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>', 'eveningshade' ).'&#8230;';
                $this->strings['unpack_package']      = __( 'Unpacking the package', 'eveningshade' ).'&#8230;';
                $this->strings['installing_package']  = __( 'Installing the plugin', 'eveningshade' ).'&#8230;';
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'eveningshade' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'eveningshade' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'eveningshade' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'eveningshade' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'eveningshade' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'eveningshade' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'eveningshade' ) . '</span><span class="hidden">' . __( 'Hide Details', 'eveningshade' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'eveningshade' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'eveningshade' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'eveningshade' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'eveningshade' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'eveningshade' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'eveningshade' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'eveningshade' ) . '</span><span class="hidden">' . __( 'Hide Details', 'eveningshade' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'eveningshade' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'eveningshade' );
                }
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'eveningshade' ) . '">' . __( 'Return to the Dashboard', 'eveningshade' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
