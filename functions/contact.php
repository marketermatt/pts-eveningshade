...
function pts_contact_form($email) {
	...
	if(isset($emailSent) && $emailSent == true) {
		...
		$out .= '<p class="thanks">'. __('<strong>Thanks!</strong> Your email was successfully sent.','eveningshade').'</p>';
		
	} else {
		
		if(isset($captchaError)) {
			$out .= '<a name="contact_"></a>';
			$out .= '<p class="error">'.__('There was an error submitting the form.','eveningshade').'</p>';
		}
		
		$out .= '<a name="contact_"></a>';
		$out .= '<form action="' .get_permalink(). '#contact_" id="contact_form" method="post">';
		$out .= '<p><label class="textfield_label" for="contactName">'.__('Name','eveningshade').' *</label><br /><input type="text" name="contactName" id="contactName" value="';
		...
		$out .= '<p><label class="textfield_label" for="email">'.__('Email','eveningshade').' *</label><br /><input type="text" name="email" id="email" value="';
		...
		$out .= '<p><label class="textfield_label" for="contactName">'.__('Comments','eveningshade').' </label><br /><textarea name="comments" id="commentsText" rows="20" cols="30" tabindex="3" class="requiredFieldContact textarea';
		...
		$out .= '<p class="screenReader"><label for="checking" class="screenReader">'.__('If you want to submit this form, do not enter anything in this field','eveningshade').'</label><input type="text" name="checking" id="checking" class="screenReader" value="';
